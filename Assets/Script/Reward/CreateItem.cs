...
public class CreateItem : MonoBehaviour
{
    ...
    [Header("강화 포션 스폰")]
    [SerializeField] private int _dropCouponInfra = 10; // 기본 드랍
    [SerializeField] private int _dropCouponRate = 5;  // 계수
    [SerializeField] private int _PetdropCouponInfra = 1; // 기본 드랍
    [SerializeField] private int _PetdropCouponRate = 5;  // 계수
    ...
    public void ItemDrop(Vector3 dropPosition)
    {
        ...
        {
            case ItemType.None:
                _damageLogUI.AnyLog("잔해속에는 아무것도 없습니다");
                break;

            case ItemType.Key:
                DropKey(dropPosition);
                _damageLogUI.AnyLog("잔해속에서 열쇠를 찾았습니다");
                break;

            case ItemType.Powerpotion:
                DropPotion(dropPosition);
                _damageLogUI.AnyLog("잔해속에서 엘릭서를 찾았습니다");
                break;

            case ItemType.PlayerUpgrade:
                DropPlayerUpgrade(out int coupon);
                _damageLogUI.AnyLog($"잔해속에서 강화 쿠폰 {coupon}를 찾았습니다 ");
                break;

            case ItemType.PetUpgrade:
                DropPetUpgrade(out int petcoupon);
                _damageLogUI.AnyLog($"잔해속에서 펫 강화 쿠폰 {petcoupon}를 찾았습니다 ");
                break;
    ...
    public void UpgradeEnermy()
    {
        ...
        _damageLogUI.AnyLog($"<color=yellow>충분한 열쇠가 모여 상자의 레벨이 오릅니다</color>");
