...
public class BossUI : MonoBehaviour
{
    ...
    private void EndTextUpdate()
    {
        ...
        _attackPower.text = $"공격력 : {_playerData.AttackPower:F0}";
        _attackRange.text = $"사거리 : {_playerData.AttackRange:F0}";
        _attackSpeed.text = $"공격 속도 : {_playerData.AttackSpeed:F0}";
        _criticalChance.text = $"치명타 확률 : {_playerData.CriticalChance * 100:F2}";
        _criticalDamageMultiplier.text = $"치명타 배율 : {_playerData.CriticalDamageMultiplier * 100:F2}";
        _moveSpeed.text = $"이동속도 : {_playerData.MoveSpeed:F2}";
        _rotateSpeed.text = $"회전속도 : {_playerData.RotateSpeed:F0}";
        ...
        _totalDamage.text = $"가한 데미지 : {_playerData.TotalDamage:F0}";
