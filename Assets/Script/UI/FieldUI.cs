...
public class FiledUI : MonoBehaviour
{
    ...
    private void BottomBarTextUpdate()
    {
        ...
        _attackSpeed.text = $"공격 속도 : {_playerData.AttackSpeed:F2}";
    ...
    private void PowerTimer()
    {
        if(_useItem.FlagPotionTimer)
        {
            ...
            _powerTimeText.text = $"남은 시간 : {_useItem.PowerTimer:F1}";
        }
        else
        {
    ...
    private void OnButtonClick(string buttonType)
    {
        ...
        {
            _upgradeCost.text = $"소모 쿠폰값 : {(_playerData.UpgradeCost)}";
            _soundManager.SFXStartMenuPlay();
            OpenUpgradeUI();
        }
    ...
    private void BossSceneMove()
    {
        if(_gameData.CurrentPhase == GameData.GamePhase.BossBattle)
        {
            ...
            if (_bossLogTimer >= 1f)
            {
                ...
                _damageLogUI.AnyLog("<color=red>보스 등장 이동해야합니다</color>");
            }
        }
        else
        {
